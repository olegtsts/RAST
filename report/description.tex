...
\begin{itemize}
	\item $ShardData$, which is a type of data stored in shard
	\item $ShardGroupData$, which is a common data for a group of shards belonging to one thread
\end{itemize}

Moreover, user would provide $Sharder$ with several lambda-functions:
\begin{itemize}
	\item $ProcessShard(can\_be\_updated, shard\_data, shard\_group\_data)$ which is called on shard processing
	\item $ProcessShardGroup(can\_be\_updated, shard\_group\_data)$ which is called on shard group processing
	\item $InitialShardCallback(target\_shard\_configuration, thread\_count)$ which is an implementation of an algorithm which constructs the initial sharding
	\item $ReshardCallback(old\_shard\_configuration, new\_shard\_configuration, thread\_count)$, which is an implementation of an algorithm of how user wants to do resharding
\end{itemize}
...
\begin{itemize}
	...
	\item call $ProcessShard$ and $ProcessShardGroup$ functions to process shards actions
\end{itemize}
...
\begin{itemize}
	\item $data$ of type $ShardData$ (provided by user via template)
	\item mutex
\end{itemize}

$Shard$ will have the following functions:
\begin{itemize}
	\item $Lock$ which locks the mutex of the shard
	\item $Unlock$ which unlocks the mutex
\end{itemize}

Let us introduce a $ShardGroup$, which consist of the following:
\begin{itemize}
	\item $can\_be\_updated$ stores whether a thread had already made a promise not to update this shard group
	\item $data$ stores a user-defined group data of type $ShardGroupData$
	\item vector $shards$ of $Shard$ instances
\end{itemize}

$ShardingConfiguration$ would contain a $shard\_groups$ vector of $ShardGroup$ instances.

$Sharder$ contains the following variables:
\begin{itemize}
	\item $first\_conf$ of type $ShardingConfiguration$
	\item $second\_conf$ of type $ShardingConfiguration$
...
\begin{itemize}
	\item $threads\_count$ is initialized as maximum hardware concurrency
	\item $(is\_first\_main, noupdate\_counter) = (true, 0)$
	\item $first\_conf$ is initialized by call to $InitialShardCallback(first\_conf, thread\_count)$
...
\begin{itemize}
	...
	\item if $noupdate\_counter$ is less than $threads\_count$, do nothing
	\item call $ReshardCallback(GetConf(is\_first\_main), GetConf(!is\_first\_main), thread\_count)$, which fills shadow configuration $GetConf(!is\_first\_main)$ by some algorithm basing on the current configuration $GetConf(is\_first\_main)$
...
\begin{itemize}
	\item sets $GetConf(is\_first\_local).shard\_groups[thread\_num].can\_be\_updated$ to $false$.
...
\begin{itemize}
	...
	\item return $GetConf(is\_first\_local).shard\_groups[thread\_num]$
\end{itemize}

$SwitchConfiguration$ procedure does the following:
\begin{itemize}
	\item call $reshard\_waiting\_timer.Reset()$
	\item for all shards in $GetConf(is\_first\_local).shard\_groups[thread\_num].shards$ call $Unlock$
	\item for all shards in $GetConf(is\_first\_main).shard\_groups[thread\_num].shards$ call $Lock$
...
\begin{itemize}
	...
	\item call $GetShardGroup$ and store value to $group$ variable
	\item call $ProcessShardGroup(group.can\_be\_updated, group.data)$, wrapping exceptions with retries
	\item for each shard in group calls $ProcessShard(group.can\_be\_updated, shard.data, group.data)$, wrapping exceptions with retries
...
\begin{itemize}
	\item $ShardData$ of different shards cannot manipulate the same external not-thread-safe data, otherwise a data race would occur
	\item however, for the reason that shards are protected with mutex, $ShardData$ of each single shard may contain external links and operate with external not-thread-safe data
	\item $ShardGroupData$ cannot manipulate with references to external not-thread-safe data at all, because it is not protected with mutexes.
	\item $ShardData$ and $ShardGroupData$ may store any inner not-thread-safe data with no negative effect, because new shards and new shard groups are constructed over shadow page of sharding configuration
	\item since we are retrying $ProcessShardGroup$ and $ProcessShard$ functions, they must provide at least basic exception safety
\end{itemize}

Exception requirements:
\begin{itemize}
	\item $InitialShardCallback$ may throw, should provide basic exception safety
	\item $ReshardCallback$ should not throw
\end{itemize}
\newpage
...
protected:
	...
	public:
		...
		// Calls Receive method of MessageProcessor, passing as SenderProxy object
		// May throw, basic exception safety
	};
public:
...
Here are how helper classes look:
\begin{lstlisting}
template <typename ShardData>
class Shard {
	void Lock() noexcept;
	// Although lock of the mutex can throw,
	// it does throw in cases if our thread has
	// no permissions to lock mutex, or if
	// a deadlock is detected.
	// We treat both cases as being impossible
...
template <typename ShardData, typename ShardGroupData>
class ShardGroup {
public::
	bool can_be_updated;
	ShardGroupData data;
	std::vector<Shard<ShardData>> shards;
};

template <typename ShardData, typename ShardGroupData>
class ShardingConfiguration {
public:
	std::vector<ShardGroup<ShardData, ShardGroupData> shard_groups;
};
\end{lstlisting}
...
class Sharder {
public:
	using Conf = ShardingConfiguration<ShardData, ShardGroupData>;

	template <typename TInitializeFunc>
	Sharder(TInitializeFunc initial_shard_callback);
	// Constructor may throw, basic exception safety

	Conf& GetConf(bool is_first) noexcept;

	template <typename TReshardFunc>
	void Reshard(TReshardFunc reshard_callback) noexcept;
	// Required reshard_callback to not throw

	void NoUpdatePromise() noexcept;

	ShardGroup& GetShardGroup() noexcept;

	void SwitchConfiguration() noexcept;

	template <typename TProcessShardFunc, typename TProcessShardGroupFunc, typename TReshardFunc>
	void ThreadAction(const int thread_num, TProcessShardFunc process_shard,
		TProcessShardGroupFunc process_shard_group, TReshardFunc reshard) noexcept;
	// Inside we have to catch exceptions, though

	template <typename TProcessShardFunc, typename TProcessShardGroupFunc, typename TReshardFunc>
	void Run(TProcessShardFunc process_shard, TProcessShardGroupFunc process_shard_group,
		TReshardFunc reshard) noexcept;
