...
\subsection{Sorted values mapping}
\label{sorted_values_mapping}

Here we are to introduce a container \\
$SortedValuesMapping<TKey, TValue, THash=std{::}hash<Key>>$, which stores key-value mapping as well as maintains a sorted order on values.
...
$SortedValuesMapping$ would store the following data:
\begin{itemize}
...
$SortedValuesMapping$ would have the following functions:
\begin{itemize}
