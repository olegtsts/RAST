...
We would use $ExceptionWithBacktrace$ class of exception defined further in the section \fullref{subsec:exception_with_backtrace}. This exception class accesses global $ExceptionTopKeeper$ instance in its constructor in order to register itself via $AddException$ method.

\begin{lstlisting}
class ExceptionTopKeeper {
public:
	void Dump() const noexcept;
	// Dumps exception top to file in path

	void Restore();
	// Restores previous exception top from file

	void SetPath(const std::string& new_path) noexcept;
	// Sets path to new_path

	void AddException(const ExceptionWithBacktrace& exception) noexcept;
	// Adds exception to form top
private:
	std::string path;
	std::unordered_map<ExceptionWithBacktrace, uint64_t, ExceptionWithBacktraceHasher> exception_frequency;
	std::set<std::pair<uint64_t, ExceptionWithBacktrace>> frequency_exception;
