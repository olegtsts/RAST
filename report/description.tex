...
\begin{itemize}
	...
	\item $last\_duration$ which stores last measured duration
...
\begin{itemize}
	\item decrease $last\_duration$ by $start\_timer.GetPassedTime()$ (method $GetPassedTime$ is defined in the \fullref{subsec:low_overhead_periodic_timer} section)
\end{itemize}

Algorithm for $Finish$ method
\begin{itemize}
	\item increase $last\_duration$ by $finish\_timer.GetPassedTime()$
	\item increase $duration\_sum$ by $last\_duration$
...
\subsection{Ranked map}
\label{ranked_map}

Here we are to introduce a container \\
$RankedMap<TKey, TValue, THash=std{::}hash<Key>>$, which stores key-value mapping as well as maintains a sorted order on values.

There are the following requirements on template class arguments:
\begin{itemize}
	\item $THash$ should have $operator()$ function, which accepts $TKey$ instance and returns hash function value
	\item $TKey$ should have comparators $operator{==}$, $operator{<}$
	\item $TValue$ should have comparator $operator{<}$
\end{itemize}
...
\begin{itemize}
	...
	\item ranked map $exception\_frequency$ with key being a string of exception and value being equal exception frequency
	\item ranked map $exception\_last\_time$ with exception as key and its last appearance time as a value
...
\begin{itemize}
	...
	\item $SetPath(new\_path)$ which changes $path$ value to the one user wants and tries to read old exception data from it if exists
...
\label{subsec:dynamically_sharded_message_passing_pool}
...
\label{subsec:finite_state_machine}

In order to build a consensus algorithm over several different hosts we have to design a FSM it is going to use to synchronize data it works with. Finite state machine would be represented as an object of some class, which works with so called write-ahead-log, which is a list of updates, which are going to be applied to the final state.

Our finite state machine is going to be used as part of \fullref{subsec:dynamically_sharded_message_passing_pool}, that is why it has to be designed as a \fullref{subsec:message_processor}

Assume we already have $TState$ class, which represents an object state, and a $TUpdate$ class, which represents an update element, which can be applied to state by calling $update.Update(state)$. Also, update can be reverted by calling $update.Revert(state)$.

Let us introduce a class $FSMMP<TController, TState, TUpdate>$, which is inherited from $MessageProcessorBase$. $TController$ represents another message processor type, which is controlling current state machine processor by sending messages.
...
$Receive(ReceivingFrom<TController>(), get\_upcoming\_update, sender)$ method does the following:
\begin{itemize}
	\item creates empty $fsm\_updates$ of type $FSMUpdatesMessage$
	\item fills $fsm\_updates \rightarrow updates$ list with states from iterator $cleanup\_border$ to the end
	\item calls $sender.Send<TController>(updates)$
\end{itemize}

This message processing may throw on allocating new $fsm\_updates$, and may throw in $sender.Send<TController>(updates)$. In either cases, method has no effect. This implies it guarantees strong exception safety.

$Receive(ReceivingFrom<TController>(), revert, sender)$ method does the following:
\begin{itemize}
	\item calls $Revert$
\end{itemize}

This message processing provides basic exception safety and is retryable.

\subsection{RAFT Core}

In order to synchronize state between several hosts, we are to build up a Replicate State Machine using RAFT algorithm.

Let us introduce $RAFTCoreMP<TController>$ class, which is a \fullref{subsec:message_processor}, which controls consensus algorithm.
...
%$Receive(ReceivingFrom<TController>(), follower\_response\_message, sender)$ does the following:
%\begin{itemize}
%	\item update $hosts\_info[follower\_response\_message.hostname].max\_saved\_transaction\_id$ to $follower\_response\_message.max\_saved\_transaction\_id$
%	\item calculate $min\_max\_saved\_transaction\_id$, which is a  minimum of $max\_saved\_transaction\_id$ for all hosts in $hosts\_info$
%	\item if $max\_fixed < write\_ahead\_log.end()$ and next WAL entry after $max\_fixed$ has $transaction\_id$, which is less, than $min\_max\_saved\_transaction\_id$
%\end{itemize}
...
private:
	std::string path;
	RankedMap<std::string, uint64_t> exception_frequency;
...
Here we are to outline an interface for \fullref{subsec:finite_state_machine}.

\begin{lstlisting}
class FSMCommitMessage {};

class GetFSMUpcomingUpdateMessage {};

class FSMUpcomingRevertMessage {};

template <typename TUpdate>
class FSMUpdatesMessage {
public:
	std::list<std::unique_ptr<TUpdate>> updates;
};

template <typename TController, typename TState, typename TUpdate>
class FSMMP {
public:
	...
	void AddUpdates(std::list<std::unique_ptr<TUpdate>&) noexcept;

	void Revert();
	// May throw, basic exception safety

	<template Sender>
	void Receive(const ReceivingFrom<TController>&, const FSMCommitMessage&, const Sender& sender) noexcept;

	<template Sender>
	void Receive(const ReceivingFrom<TController>&, const FSMUpdatesMessage&, const Sender& sender) noexcept;

	<template Sender>
	void Receive(const ReceivingFrom<TController>&, const GetFSMUpcomingUpdateMessage&, const Sender& sender);
	// May throw, basic exception safety

	<template Sender>
	void Receive(const ReceivingFrom<TController>&, const FSMUpcomingRevertMessage&, const Sender& sender);
	// May throw, basic exception safety
private:
	std::list<std::unique_ptr<TUpdate>> write_ahead_log;
	typename std::list<std::unique_ptr<TUpdate>>::iterator cleanup_border;
	typename std::list<std::unique_ptr<TUpdate>>::iterator applied_border;
	TState state;
};
\end{lstlisting}
\end{document}
