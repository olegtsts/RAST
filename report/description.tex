...
Let us introduce $RAFTCoreMP$ class, which is a \fullref{subsec:message_processor}, which controls consensus algorithms.

$RAFTCoreMP$ stores the following data:
\begin{itemize}
...
\begin{itemize}
	\item $RequestVote$, which is used to ask for a vote in case if some other host is processing a leader election
	\item $ResponseVode$, which is used to respond on vote
	\item $Masterbeat$, which contains update, transferred from master to slave hosts
	\item $FollowerResponse$, which is a response to $Masterbeat$
\end{itemize}
