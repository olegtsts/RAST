...
Here we are aiming to create a timer, which helps to schedule periodic actions. In constructor it accepts a time period value. It has $CheckTime$ function, which returns $true$ if given time passed, and $false$, if time have not passed yet. If $true$ value is returned, timer resets and we proceed to waiting for the same period of time again.
...
\begin{itemize}
	...
	\item $time\_elapsed$ which is time passed after last $CheckTime$ call, which returned true
	\item $periodic\_timer$
\end{itemize}

Let us introduce helping function $StartWaiting$ with the following logic:
\begin{itemize}
	\item call $periodic\_timer.GetPassedTime()$ to fix point since which we are going to trace time
	\item update $time\_elapsed = 0$
\end{itemize}

$StartWaiting$ should be called in the constructor of the class $WaitingTimer$, because we start waiting since the initialization of the object.

Function $CheckTime$ would have the following logic:
\begin{itemize}
	\item update $time\_elapsed += periodic\_timer.GetPassedTime()$
	\item calculate boolean $is\_time\_elapsed = time\_elapsed > time\_period$
	\item if $is\_time\_elapsed$ is true, call $StartWaiting$
	\item return $is\_time\_elapsed$
\end{itemize}
