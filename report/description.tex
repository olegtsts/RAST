...
\begin{itemize}
	...
	\item $PreProcesss(can\_be\_updated)$ which is called from $Sharder$ before processing shards
	\item $OnSwitch(new\_shards)$ which is called by $Sharder$ when a thread started to work with new shards after resharding. A new shards vector is passed to it.
...
\begin{itemize}
	...
	\item call $ProcessShard$, $PreProcesss$ and $OnSwitch$ functions to process shards actions
\end{itemize}
...
\begin{itemize}
	...
	\item call $controller.GetResharding(GetConf(is\_first\_main), thread\_count)$ and assign its result to $GetConf(!is\_first\_main)$
...
\begin{itemize}
	...
	\item call $controller.OnSwitch(GetConf(ls\_first\_local)[thread\_num])$
\end{itemize}
...
\begin{itemize}
	...
	\item call $controller.PreProcesss(can\_be\_updated)$, wrapping exceptions with retries
...
\begin{itemize}
	...
	\item $PreProcesss$ cannot manipulate with external not-thread-safe data at all, because it is not protected with mutexes.
	...
	\item while implementing $OnSwitch(new\_shards)$ it is safe to assume, that each shard of $new\_shards$ is not processed by any thread until $OnSwitch$ completion. That is because before calling $OnSwitch$, we locked the shards in the current thread, but have not started to process them yet. It is also safe to assume, that main thread is not processing function $GetResharding$ until $OnSwitch$ completion, for the reason that otherwise $OnSwitch$ is processed at the same time as $GetResharding$ is processing, which is impossible as it has been stated above.
\end{itemize}

The exception requirements:
\begin{itemize}
	\item $ProcessShard$ may throw, should provide at least basic exception safety
	\item $PreProcesss$ may throw, should provide at least basic exception safety
	...
	\item $GetResharding$ should not throw
\end{itemize}
...
class Controller {
public:
	void ProcessShard(can_be_updated, shard_num);
	// Exceptions: basic safety

	void PreProcess(can_be_updated);
	// Exceptions: basic safety

	void OnSwitch(const std::vector<int>& new_shards) noexcept;
	...
	std::vector<std::vector<int>> GetResharding(
		...
		const int thread_count) noexcept;
};
\end{lstlisting}
